...
* **Vote:** Once deposit reaches a certain value (`MinDeposit`), proposal is 
  confirmed and vote opens. Bonded BPS holders can then send `TxGovVote` 
...
Any BPS holder, whether bonded or unbonded, can submit proposals by sending a 
...
Cosmos Hub, participants are bonded BPS holders. Unbonded BPS holders and 
other users do not get the right to participate in governance. However, they 
can submit and deposit on proposals.

Note that some *participants* can be forbidden to vote on a proposal under a 
certain validator if:
* *participant* bonded or unbonded BPS's to said validator after proposal 
  entered voting period.
* *participant* became validator after proposal entered voting period.

This does not prevent *participant* to vote with BPS's bonded to other 
validators. For example, if a *participant* bonded some BPS's to validator A 
before a proposal entered voting period and other BPS's to validator B after 
...
Proposals can be accepted before the end of the voting period if they meet a special condition. Namely, if the ratio of `Yes` votes to `InitTotalVotingPower`exceeds 2:3, the proposal will be immediately accepted, even if the `Voting period` is not finished. `InitTotalVotingPower` is the total voting power of all bonded BPS holders at the moment when the vote opens. 
