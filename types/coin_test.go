...
func TestNewCoins(t *testing.T) {
	...
	tests := []struct {
		...
		{"one coin", []Coin{tenbps}, Coins{tenbps}, false},
		{"sort after create", []Coin{tenbtc, tenbps}, Coins{tenbps, tenbtc}, false},
		{"sort and remove zeroes", []Coin{zeroeth, tenbtc, tenbps}, Coins{tenbps, tenbtc}, false},
		{"panic on dups", []Coin{tenbps, tenbps}, Coins{}, true},
	}
...
func TestCoinsIsAnyGT(t *testing.T) {
	twoBps := NewInt64Coin("bps", 2)
	fiveBps := NewInt64Coin("bps", 5)
	...
	require.False(t, Coins{fiveBps}.IsAnyGT(Coins{}))
	require.False(t, Coins{}.IsAnyGT(Coins{fiveBps}))
	require.True(t, Coins{fiveBps}.IsAnyGT(Coins{twoBps}))
	require.False(t, Coins{twoBps}.IsAnyGT(Coins{fiveBps}))

	require.True(t, Coins{twoBps, sixEth}.IsAnyGT(Coins{twoBtc, fiveBps, threeEth}))
	require.False(t, Coins{twoBtc, twoBps, threeEth}.IsAnyGT(Coins{fiveBps, sixEth}))
	require.False(t, Coins{twoBps, sixEth}.IsAnyGT(Coins{twoBtc, fiveBps}))
}
