...
func TestMsgSendRoute(t *testing.T) {
	...
	coins := sdk.NewCoins(sdk.NewInt64Coin("bps", 10))
...
func TestMsgSendValidation(t *testing.T) {
	addr1 := sdk.AccAddress([]byte("from"))
	addr2 := sdk.AccAddress([]byte("to"))
	bps123 := sdk.NewCoins(sdk.NewInt64Coin("bps", 123))
	bps0 := sdk.NewCoins(sdk.NewInt64Coin("bps", 0))
	bps123eth123 := sdk.NewCoins(sdk.NewInt64Coin("bps", 123), sdk.NewInt64Coin("eth", 123))
	bps123eth0 := sdk.Coins{sdk.NewInt64Coin("bps", 123), sdk.NewInt64Coin("eth", 0)}
	...
	cases := []struct {
		...
		{true, NewMsgSend(addr1, addr2, bps123)},       // valid send
		{true, NewMsgSend(addr1, addr2, bps123eth123)}, // valid send with multiple coins
		{false, NewMsgSend(addr1, addr2, bps0)},        // non positive coin
		{false, NewMsgSend(addr1, addr2, bps123eth0)},  // non positive coin in multicoins
		{false, NewMsgSend(emptyAddr, addr2, bps123)},  // empty from addr
		{false, NewMsgSend(addr1, emptyAddr, bps123)},  // empty to addr
	}
...
func TestMsgSendGetSignBytes(t *testing.T) {
	...
	expected := `{"type":"cosmos-sdk/MsgSend","value":{"amount":[{"amount":"10","denom":"bps"}],"from_address":"cosmos1d9h8qat57ljhcm","to_address":"cosmos1da6hgur4wsmpnjyg"}}`
...
func TestInputValidation(t *testing.T) {
	...
	someCoins := sdk.NewCoins(sdk.NewInt64Coin("bps", 123))
	multiCoins := sdk.NewCoins(sdk.NewInt64Coin("bps", 123), sdk.NewInt64Coin("eth", 20))

	var emptyAddr sdk.AccAddress
	emptyCoins := sdk.NewCoins()
	emptyCoins2 := sdk.NewCoins(sdk.NewInt64Coin("eth", 0))
	someEmptyCoins := sdk.Coins{sdk.NewInt64Coin("eth", 10), sdk.NewInt64Coin("bps", 0)}
	unsortedCoins := sdk.Coins{sdk.NewInt64Coin("eth", 1), sdk.NewInt64Coin("bps", 1)}
...
func TestMsgMultiSendValidation(t *testing.T) {
	...
	addr2 := sdk.AccAddress([]byte{7, 8})
	bps123 := sdk.NewCoins(sdk.NewInt64Coin("bps", 123))
	bps124 := sdk.NewCoins(sdk.NewInt64Coin("bps", 124))
	eth123 := sdk.NewCoins(sdk.NewInt64Coin("eth", 123))
	bps123eth123 := sdk.NewCoins(sdk.NewInt64Coin("bps", 123), sdk.NewInt64Coin("eth", 123))

	input1 := NewInput(addr1, bps123)
	input2 := NewInput(addr1, eth123)
	output1 := NewOutput(addr2, bps123)
	output2 := NewOutput(addr2, bps124)
	outputMulti := NewOutput(addr2, bps123eth123)
	...
	cases := []struct {
		...
		{false, MsgMultiSend{
			Inputs:  []Input{NewInput(emptyAddr, bps123)}, // invalid input
			Outputs: []Output{output1}}},
		{false, MsgMultiSend{
			Inputs:  []Input{input1},
			Outputs: []Output{{emptyAddr, bps123}}}, // invalid output
		},
		{false, MsgMultiSend{
			Inputs:  []Input{input1},
...
func TestMsgMultiSendGetSignBytes(t *testing.T) {
	...
	expected := `{"type":"cosmos-sdk/MsgMultiSend","value":{"inputs":[{"address":"cosmos1d9h8qat57ljhcm","coins":[{"amount":"10","denom":"bps"}]}],"outputs":[{"address":"cosmos1da6hgur4wsmpnjyg","coins":[{"amount":"10","denom":"bps"}]}]}}`
	require.Equal(t, expected, string(res))
}
