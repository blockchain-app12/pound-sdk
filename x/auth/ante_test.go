...
func TestAnteHandlerFees(t *testing.T) {
	...
	acc1.SetCoins(sdk.NewCoins(sdk.NewInt64Coin("bps", 149)))
	input.ak.SetAccount(ctx, acc1)
	checkInvalidTx(t, anteHandler, ctx, tx, false, sdk.CodeInsufficientFunds)

	require.True(t, input.sk.GetModuleAccount(ctx, types.FeeCollectorName).GetCoins().Empty())
	require.True(sdk.IntEq(t, input.ak.GetAccount(ctx, addr1).GetCoins().AmountOf("bps"), sdk.NewInt(149)))

	acc1.SetCoins(sdk.NewCoins(sdk.NewInt64Coin("bps", 150)))
	input.ak.SetAccount(ctx, acc1)
	checkValidTx(t, anteHandler, ctx, tx, false)

	require.True(sdk.IntEq(t, input.sk.GetModuleAccount(ctx, types.FeeCollectorName).GetCoins().AmountOf("bps"), sdk.NewInt(150)))
	require.True(sdk.IntEq(t, input.ak.GetAccount(ctx, addr1).GetCoins().AmountOf("bps"), sdk.NewInt(0)))
}
...
func TestAnteHandlerMemoGas(t *testing.T) {
	...
	msg := types.NewTestMsg(addr1)
	privs, accnums, seqs := []crypto.PrivKey{priv1}, []uint64{0}, []uint64{0}
	fee := NewStdFee(0, sdk.NewCoins(sdk.NewInt64Coin("bps", 0)))
...
func TestCustomSignatureVerificationGasConsumer(t *testing.T) {
	...
	priv1, _, addr1 := types.KeyTestPubAddr()
	acc1 := input.ak.NewAccountWithAddress(ctx, addr1)
	_ = acc1.SetCoins(sdk.NewCoins(sdk.NewInt64Coin("bps", 150)))
	input.ak.SetAccount(ctx, acc1)
	...
	acc2 := input.ak.NewAccountWithAddress(ctx, addr2)
	require.NoError(t, acc2.SetCoins(sdk.NewCoins(sdk.NewInt64Coin("bps", 150))))
	require.NoError(t, acc2.SetAccountNumber(1))
	input.ak.SetAccount(ctx, acc2)
	msg = types.NewTestMsg(addr2)
	privs, accnums, seqs = []crypto.PrivKey{priv2}, []uint64{1}, []uint64{0}
